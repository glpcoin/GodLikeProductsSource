...
const char * const BITCOIN_CONF_FILENAME = "godlikeproducts.conf";
const char * const BITCOIN_PID_FILENAME = "godlikeproductsd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "godlikeproducts" is a composite category enabling all GodLikeProducts-related debug output
                if(ptrCategory->count(std::string("godlikeproducts"))) {
...
#else
    const char* pszModule = "godlikeproducts";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GodLikeProducts
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GodLikeProducts
    // Mac: ~/Library/Application Support/GodLikeProducts
    // Unix: ~/.godlikeproducts
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GodLikeProducts";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GodLikeProducts";
#else
    // Unix
    return pathRet / ".godlikeproducts";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty godlikeproducts.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override godlikeproducts.conf
